...
def to_rigid_object_dataset(cosypose_object_ds):
    """Convert cosypose dict-based BOPObjectDataset to toolbox RigidObjectDataset.

    Preserves original labels (no dataset prefix) to match cosypose model expectations.
    """
    objects = [
        RigidObject(
            label=obj["label"],
            mesh_path=Path(obj["mesh_path"]),
            mesh_units=obj["mesh_units"],
            mesh_diameter=obj.get("diameter"),
        )
        for obj in cosypose_object_ds.objects
    ]
...
def load_pose_models(coarse_run_id, refiner_run_id=None, n_workers=8):
    ...
    renderer = Panda3dBatchRenderer(asset_dataset=to_rigid_object_dataset(object_ds), n_workers=n_workers)
